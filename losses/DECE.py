...
class DECE(nn.Module):
    ...
    def __init__(self, num_bins, t_a, t_b):
    ...
    def __call__(self, input, target):
        ...
        cut_points = torch.linspace(0, 1, self.num_bins + 1)[:-1].to(device=input.get_device())
        W = torch.reshape(
            torch.linspace(1.0, self.num_bins, self.num_bins).to(device=input.get_device()),
            [1, -1],
        )

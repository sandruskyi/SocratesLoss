...
__all__ = [
    'BasicBlock', 'Bottleneck', 'ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet110', 'resnet152'
]
...
class ResNet(nn.Module):
    name = "resnet"
    def __init__(self, block, num_blocks, num_classes=10, temp=1.0, input_size=-1):
        ...
        if input_size == 32:
            ...
            self.fc = nn.Linear(512*7*7*block.expansion, num_classes)
        self.temp = temp
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        #out = F.avg_pool2d(out, 4) #old
        out = self.avgpool(out) # new (csc)
...
def resnet34(dataset_name, temp=1.0, **kwargs):
    model = ResNet(BasicBlock, [3, 4, 6, 3], temp=temp, **kwargs)
    return model


def resnet50(dataset_name, temp=1.0, **kwargs):
...
def resnet110(dataset_name, temp=1.0, **kwargs):

...
#          u-Chang Wu, Shen-Huan Lyu, Haopu Shang, Xiangyu Wang, Chao Qian (2024). Confidence-aware Contrastive Learning
#          for Selective Classification. International Conference on Machine Learning
# from https://github.com/lamda-bbo/CCL-SC/tree/main
####################################################################################
import torch
...
class MoCo(nn.Module):
    ...
    def __init__(self, dim=128, K=65536, m=0.999, T=0.07, num_class=10, mlp=False, args=None):
        ...
        self.register_buffer("prediction_queue",
                             torch.randint(low=0, high=num_class, size=(self.K2,), dtype=torch.long))
        ...
        self.register_buffer("correct_prediction_queue",
                             torch.randint(low=0, high=num_class, size=(K,), dtype=torch.long))
    ...
    def _batch_shuffle_ddp(self, x):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        if self.args != None and self.args.clusterLessPower:
            primary_device = list(map(int, self.args.gpu_id.split(',')))[0]
    ...
    def forward(self, hidden_features, hidden_features_k, targets, outputs, outputs_k, epoch, pretrain, full_flag):
        ...
        predicted_k = predicted_k.to(device)

        correct_mask_k = (predicted_k == targets.to(device))
        ...
        correct_mask = (predicted.to(device) == targets.to(device))
        ...
        with torch.no_grad():  # no gradient to keys
            hidden_features_k = hidden_features_k.to(device)
            k_error = hidden_features_k[~correct_mask_k].to(device)  # keys: NxC
            k_error = nn.functional.normalize(k_error, dim=1)

            k_correct = hidden_features_k[correct_mask_k].to(device)
            k_correct = nn.functional.normalize(k_correct, dim=1)

        if epoch >= pretrain and full_flag:
            # compute query features
            q = hidden_features.to(device)  # queries: NxC
            q = nn.functional.normalize(q, dim=1)

            # TF, not sim
            sim_matrix = torch.matmul(q, self.queue.detach().to(device))
            eq_matrix = torch.eq(targets.view(-1, 1), self.prediction_queue.detach().view(1, -1).to(device))
            sim_matrix *= eq_matrix
            sim_matrix += (eq_matrix == False).float() * -1e9
            del eq_matrix

            # TT, sim
            sim_matrix_tp = torch.matmul(q, self.correct_queue.detach().to(device))
            eq_matrix_tp = torch.eq(targets.view(-1, 1), self.correct_prediction_queue.detach().view(1, -1).to(device))
            sim_matrix_tp *= eq_matrix_tp
            sim_matrix_tp = sim_matrix_tp[eq_matrix_tp]
            non_zero_counts = (eq_matrix_tp != 0).sum(dim=1).to(device)
            ...
            if error_predicts.shape[0] == 0 or True:
                ...
                return full_k1, full_k2, sum(correct_mask_k).item() / int(targets.size().numel()), (
                            self.T / self.base_temperature) * info_nce_loss_t

            fq = hidden_features[~correct_mask].to(device)
            fq = nn.functional.normalize(fq, dim=1)

            sim_matrix_ft = torch.matmul(fq, self.correct_queue.detach().to(device))
            eq_matrix_ft = torch.eq(error_predicts.view(-1, 1),
                                    self.correct_prediction_queue.detach().view(1, -1).to(device))
            sim_matrix_ft *= eq_matrix_ft
            sim_matrix_ft += (eq_matrix_ft == False).float() * -1e9

            # FF, sim
            sim_matrix_ff = torch.matmul(fq, self.correct_queue.detach().to(device))
            eq_matrix_ff = torch.eq(targets[~correct_mask].view(-1, 1),
                                    self.correct_prediction_queue.detach().view(1, -1).to(device))
            ...
            return full, sum(correct_mask_k).item() / int(targets.size().numel()), (self.T / self.base_temperature) * (
                        info_nce_loss_t + info_nce_loss_f)
            # dequeue and enqueue
